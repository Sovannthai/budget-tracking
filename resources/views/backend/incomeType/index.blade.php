    ...
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            @include('backend.components.table.header_table', [
                'icon_name'        => 'fa-money-bill',
                'header_text'      => __('Income Types'),
                'sub_header_title' => __('Manage your income types here'),
                'btn_title'        => __('Add Income Type'),
            ])
            <a href="#" class="btn btn-primary float-end text-capitalize" data-bs-toggle="modal"
                data-bs-target="#create_income_type">
                <i class="fa fa-plus-circle"></i> @lang('Add New')</a>
            @include('backend.incomeType.create', ['customers' => $customers])
        </div>
        <div class="card-body table-wrapper">
                ...
                <table class="table text-nowrap table-hover align-middle">
                    ...
                    <body>
                        @forelse ($incomeTypes as $row)
                            <tr>
                                ...
                                <td>
                                    <a href="#" class="me-2" data-bs-toggle="modal"
                                        data-bs-target="#edit_income_type-{{ $row->id }}" title="@lang('Edit')">
                                    ...
                                    @include('backend.components.table.action_btn', [
                                        'route_delete'   => 'income-types.destroy',
                                        'row'            => $row,
                            ...
                            @include('backend.incomeType.edit', [
                                'incomeType' => $row,
                                'customers'  => $customers,
                            ])
                        @empty
            ...
            @if (count($incomeTypes) != 0)
                @include('backend.components.table.paginate', ['items' => $incomeTypes])
            @endif
        </div>
    </div>
@endsection
